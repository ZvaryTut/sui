...
impl BenchmarkBank {
    ...
    pub async fn generate(
        ...
        for chunk in chunked_coin_configs {
            ...
            println!("Successfully adding {} gas coins...", gas_coins.len());
    ...
    async fn create_init_coin(&mut self, amount: u64, gas_price: u64) -> Result<Gas> {
        println!("Creating init coin of value {amount}...");
        ...
        match effects.created().get(0) {
            Some(created_coin) => {
                let address = created_coin.1.get_owner_address()?;
                let keypair = self.primary_coin.2.clone();
                return Ok((created_coin.0, address, keypair));
            }
            None => panic!("No coin created for workload."),
        }
    }
}
